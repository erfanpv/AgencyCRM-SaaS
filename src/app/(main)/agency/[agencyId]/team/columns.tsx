...
const TeamTableColumns: ColumnDef<UsersWithAgencySubAccountPermissionsSidebarOptions>[] =
  [
    {
      accessorKey: 'id',
      header: '',
            ...
            <div className="flex flex-col gap-2">
              {ownedAccounts?.length ? (
                ownedAccounts.map(account => (
                  ...
                  >
                    Sub Account - {account.subAccounts.name}
                  </Badge>
                ))
              ) : (
          ...
          {rowData.role !== 'AGENCY_OWNER' && (
            <AlertDialogTrigger asChild>
              <DropdownMenuItem className="flex gap-2" onClick={() => {}}>
                <Trash className="h-4 w-4" /> Remove User
              </DropdownMenuItem>
            </AlertDialogTrigger>
          )}
        </DropdownMenuContent>
...
export default TeamTableColumns;
